...
const tierColor = (t) => t === 'T1'
  ? {bg:'#e8f4fc', color:'#0c447c'}
  : t === 'T2'
  ? {bg:'#f0f7e0', color:'#3b6d11'}
  : {bg:'#fff3cd', color:'#b06a00'}
...
export default function Wassington({ orders = [], onApprove, onReject }) {
  ...
  const confirmApprove = () => { onApprove(modal.order.id, editPrice); closeModal() }
  const confirmReject  = () => { onReject(modal.order.id, reason);     closeModal() }
  ...
  const TABS = [
    { id:'orders',  label:'📦 Pedidos y aprobación' },
                  ...
                  {pending.map((o, i) => {
                    ...
                    return (
                      <tr key={i} style={{borderBottom:'0.5px solid #ddddd5'}}>
                        ...
                        <td style={{padding:'12px 16px'}}><span style={{background:tc.bg, color:tc.color, fontSize:'11px', fontWeight:700, padding:'2px 8px', borderRadius:'100px'}}>{o.tier}</span></td>
                        <td style={{padding:'12px 16px', color:'#6b6b6b'}}>{o.room}</td>
                        <td style={{padding:'12px 16px'}}><span style={{background:o.product==='MatriPowder'?'#f0f7e0':'#eaf7ee', color:o.product==='MatriPowder'?'#3b6d11':'#1a6b30', fontSize:'11px', fontWeight:700, padding:'2px 8px', borderRadius:'100px'}}>{o.product}</span></td>
                        ...
                        <td style={{padding:'12px 16px'}}><span style={{background:'#f5f5ee', color:'#6b6b6b', fontSize:'11px', fontWeight:600, padding:'2px 8px', borderRadius:'100px'}}>{o.model}</span></td>
                ...
                <tbody>
                  {processed.map((o, i) => (
                    <tr key={i} style={{borderBottom: i < processed.length-1 ? '0.5px solid #ddddd5' : 'none'}}>
                      ...
                      <td style={{padding:'12px 16px', color:'#6b6b6b'}}>{o.room}</td>
                      <td style={{padding:'12px 16px', fontWeight:700}}>${o.price}</td>
                      <td style={{padding:'12px 16px'}}><span className={`status ${o.status}`}>{o.status === 'approved' ? '✓ Aprobado' : '✗ Rechazado'}</span></td>
              ...
              {CUSTOMERS.map((c, i) => {
                ...
                return (
                  <tr key={i} style={{borderBottom: i < CUSTOMERS.length-1 ? '0.5px solid #ddddd5' : 'none'}}>
                    ...
                    <td style={{padding:'12px 16px'}}><span style={{background:tc.bg, color:tc.color, fontSize:'11px', fontWeight:700, padding:'2px 8px', borderRadius:'100px'}}>{c.tier}</span></td>
          ...
          <div style={{background:'#fff', borderRadius:'14px', padding:'28px', width:'100%', maxWidth:'420px', boxShadow:'0 8px 32px rgba(11,67,88,.2)'}}>
            {modal.action === 'approve' ? (
              <>
                ...
                <div style={{fontSize:'13px', color:'#888', marginBottom:'20px'}}>#{modal.order.id} · {modal.order.customer} · {modal.order.room}</div>
                <div style={{marginBottom:'18px'}}>
                  <label style={{display:'block', fontSize:'13px', fontWeight:500, color:'#0b4358', marginBottom:'5px'}}>Precio final (USD)</label>
                  ...
                  <div style={{fontSize:'11px', color:'#888', marginTop:'4px'}}>Precio indicativo: ${modal.order.price}. Podés confirmarlo o ajustarlo.</div>
                </div>
              ...
              <>
                ...
                <div style={{marginBottom:'18px'}}>
                  ...
                  <div style={{fontSize:'11px', color:'#888', marginTop:'4px'}}>El cliente verá este motivo en su sección de Pedidos.</div>
                </div>
                <div style={{display:'flex', gap:'10px'}}>
