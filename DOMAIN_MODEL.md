...
52. **Self-service Customer organization deletion (2026-08-25)** — a Distributor/Sub-distributor/Global's own Owner/Aprobador can permanently delete a Customer organization from the CRM — Clientes screen (`Organizations.jsx`), instead of needing a one-off SQL script run by hand each time (which is exactly how the pre-launch test-data cleanup was done earlier the same day — see project memory `prelaunch_data_cleanup`). `delete_customer_organization(p_org_id)` (migration 0035) enforces, server-side (not just a UI gate): caller holds Owner or Aprobador, and the target is within the caller's own subtree (`is_in_subtree`) and is specifically `org_type = 'customer'`. **Deliberately scoped to Customer org_type only** — deleting a Distributor/Sub-distributor (which would cascade through every org nested below it) is out of scope for this version; the button and RPC both refuse anything else. Cascades through every table a Customer org can touch (Treatments and their MatriSure/Firmness/Dispatch-Checklist dependents, Season Plans and Distributor-authored drafts, Cold Rooms, pricing overrides, invites, the access request that originated it, generator history, documents, and the org's own `auth.users` logins) in one atomic transaction, temporarily disabling `trg_prevent_last_owner_delete` (Rule 18) since the org itself disappearing is exactly why its owner count is allowed to hit zero here. UI requires typing the organization's exact name to enable the confirm button — a stronger bar than a plain yes/no, given this is a one-click, permanent, self-service action unlike the manually-reviewed SQL scripts used for earlier one-off cleanups.

53. **Editable dose at approval time (2026-08-25)** — whoever approves a Treatment (Owner/Aprobador, Rule 43) can also adjust the dose itself in the same step, not just the price — a new "Dosis objetivo (ppb)" field in the approve modal (`Wassington.jsx`), defaulting to the originally-submitted dose, with a live-recomputed sachet/tablet combo and indicative price shown as it's edited (same math `Calculator.jsx`/`SeasonPlan.jsx` use). Dose and price stay independently editable — changing one never auto-overwrites the other, same as price editing already worked before this. `approveTreatment()` (Portal.jsx) uses whichever dose is actually approved (not necessarily the original) to compute and freeze `pouch_breakdown` for Powder, and persists it to `treatments.target_dose_ppb` — for Tablets there's no separate frozen breakdown column at all (`pouchBreakdownDisplay` always recomputes live from `target_dose_ppb`, Rule 37's freeze only ever applied to Powder), so updating that one column is already sufficient there. No RLS/migration change needed — the existing approval-role trigger (Rule 43's correction note) already permits any column to change alongside the status transition to `approved`.

54. **Retiring a Distributor or Sub-distributor (2026-08-26)** — extends Rule 52's Customer deletion to the two levels above it, but with an intentionally different shape: a Distributor/Sub-distributor's own Customers (and, for a Distributor, any Sub-distributors it has) are **not** deleted — they're reassigned to a chosen target org (`retire_organization()`, migration 0036), reachable from the same CRM — Clientes screen. Valid targets are enforced server-side: retiring a Sub-distributor allows only its own parent Distributor or a sibling Sub-distributor under that same Distributor; retiring a Distributor allows any other Distributor. Reparenting every direct child in one `update organizations set parent_id = target where parent_id = retiring_org` is what carries a Sub-distributor's own Customers along automatically when a Distributor is retired — no separate recursive step needed, since a Sub-distributor's Customers are never directly reparented themselves, they just come along with their still-intact Sub-distributor parent. Within the retiring org itself: its own staff (profiles/`auth.users`) is deleted (not migrated, to avoid silently mixing two orgs' access without a human re-inviting each one deliberately); physical assets (`kit_units`, `generators`) move to the target org (a real kit or generator doesn't stop existing just because its custodian org does); its own configuration (pricing, inventory, pouch/tablet catalog, MatriSure kit lots) is deleted rather than merged into the target's (auto-merging two independent setups risks unique-constraint collisions or silently wrong combined numbers nobody asked for). Because the Customers (and their Treatments/Season Plans/MatriSure verifications/etc.) survive this operation — unlike Rule 52's Customer deletion, where everything referencing the deleted org's staff was also being deleted in the same operation — every column across the schema that could reference one of the retiring org's own staff profiles had to be identified and nulled out first (confirmed nullable case by case; the two that weren't — `user_invites.created_by`, `generator_dispatches.dispatched_to_org_id` — have their rows deleted instead, both low-stakes/historical). Same UI confirmation pattern as Rule 52 (type the organization's exact name), plus a live count of how many child organizations are about to be reassigned. Verified live end-to-end for a Distributor retiring a Sub-distributor (using throwaway test orgs); the Global-retires-a-Distributor path shares the identical function and validation logic but hasn't been live-tested yet (needs a Global login).

55. **Single Owner per organization, with an atomic transfer (2026-09-03)** — Rule 18 previously enforced only a *minimum* of one Owner per org; multiple Owners were allowed. Now exactly one is enforced going forward: `prevent_second_owner()` (migration 0038) blocks any attempt to add a second. Since a strict single-Owner rule would otherwise make it impossible to ever *change* who the Owner is (adding a new one first would violate the max, removing the old one first would violate the min), `transfer_ownership(org_id, new_owner_profile_id)` does both atomically in one step — disabling both the min-1 and max-1 triggers only for that one operation (same pattern `delete_customer_organization`/`retire_organization` already use for their own user-cleanup step). Callable by any Owner within their own subtree, not just the org's own Owner, so an ancestor can resolve a descendant's ownership too. `Users.jsx` exposes this as a "👑 Hacer Owner" button per member — including a member who already holds Owner, whenever more than one currently does (pre-existing data predates this rule), so resolving a tie down to one specific person is always reachable.

56. **Invite-link email-confirmation redirect (bug fix, 2026-09-03)** — `AuthModal.jsx`'s signup call never passed `emailRedirectTo`, so when Supabase's email-confirmation setting is on, confirming the account silently dropped the `?invite=<token>` query param the whole invite-link flow depends on — the invited person landed in "Solicitudes de usuario pendientes" instead of being auto-assigned to the Organization/roles the invite actually specified, and whoever assigned them manually risked targeting whatever Organization happened to be selected in Usuarios' own left-hand tree at that moment, not necessarily the right one. Same bug class already fixed once in the sibling DECCO-MatriSure app. Fixed by passing `emailRedirectTo` with the invite token baked back into the URL, sourced from a new `inviteToken` prop `App.jsx` now passes down to `AuthModal`.
57. **MatriSure kit-inventory review + QC gate before release (2026-09-17)** — Juan asked me to review this portal's own MatriSure kit flow against the standalone DECCO-MatriSure spin-off's (which has a dedicated `matrisure_manager` role plus a quality-control discard step before a lot is released). Investigation found two real gaps, both fixed: (a) `Inventory.jsx`'s "Lotes de kits MatriSure" section (Rule 47's `matrisure_kit_lots`) was confusingly named next to the actual kit-unit stewardship's own "🧪 Kits MatriSure" tab even though it tracks a different thing entirely (product batches, not verification kits) — relabeled to "Lotes de producto (control de vencimiento)" with an explicit note pointing to the real kit tab, no schema/data change. (b) `release_kit_units()` (Rule 49) had no quality gate at all — Global could release a lot to a Distributor with zero chance to pull a bad unit out first. Juan's call: no new role needed (any Global Operador/Owner/Aprobador already does kit intake, per Rule 49), but releasing now requires passing through a QC step — `KitsGlobal.jsx` gained a per-unit "🗑️ Descartar" action on in-stock units (plain `kit_units` update to `status='destroyed'` with a reason, same client-side pattern `KitsDistributor.jsx` already used for its own Fase-K-2e discard, no new RPC needed since `kit_units_update`'s RLS already permits Global to touch its own `registered` units) and a mandatory "Confirmo el control de calidad de estas unidades" checkbox that gates the "Liberar" button itself. Distributor's own receiving/assignment flow (Rule 49) is unchanged — Juan confirmed the no-separate-receipt-step design is fine as-is.
